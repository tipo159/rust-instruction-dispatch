...
#endif
  memory[GET_OPERAND_A(program[program_counter])] =
      memory[GET_OPERAND_B(program[program_counter])] +
      memory[GET_OPERAND_C(program[program_counter])];
  DISPATCH();

do_jmpne:
#if defined(DEBUG)
  printf("%d: jmpne if memory[%d](%d) != memory[%d](%d) then program_counter = "
         "%d\n",
...
#endif
  if (memory[GET_OPERAND_A(program[program_counter])] !=
      memory[GET_OPERAND_B(program[program_counter])]) {
