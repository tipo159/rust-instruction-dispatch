...
do_load:
  ...
  printf("%ld: load memory[%d] = %d\n", program_pointer - &program[0],
         GET_OPERAND_A(*program_pointer), GET_OPERAND_IMM(*program_pointer));
#endif
...
do_add:
  ...
  printf("%ld: add memory[%d](%d) = memory[%d] + memory[%d]\n",
         ...
         GET_OPERAND_B(*program_pointer), GET_OPERAND_C(*program_pointer));
#endif
  memory[GET_OPERAND_A(*program_pointer)] =
      memory[GET_OPERAND_B(*program_pointer)] +
      memory[GET_OPERAND_C(*program_pointer)];
  DISPATCH();

do_jmpne:
#if defined(DEBUG)
  printf("%ld: jmpne if memory[%d](%d) != memory[%d](%d) then program_pointer "
         "= %d\n",
         program_pointer - &program[0], GET_OPERAND_A(*program_pointer),
         memory[GET_OPERAND_A(*program_pointer)],
         GET_OPERAND_B(*program_pointer),
         memory[GET_OPERAND_B(*program_pointer)],
         GET_OPERAND_JMP(*program_pointer));
#endif
  if (memory[GET_OPERAND_A(*program_pointer)] !=
      memory[GET_OPERAND_B(*program_pointer)]) {
