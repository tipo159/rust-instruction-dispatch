...
pub unsafe fn vm_loop(program: &[Bytecode]) {
    ...
    let mut dispatch_sequence: [usize; 256] = [0; 256];
    let mut i: usize = 0;

    loop {
        dispatch_sequence[i] = jump_table[get_opcode((*program)[i]) as usize];
        i += 1;
        if get_opcode((*program)[i - 1]) == Opcode::RET {
            break;
        }
    }

    dispatch!(pc, dispatch_sequence);
